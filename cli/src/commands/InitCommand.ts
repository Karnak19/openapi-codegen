...
export class InitCommand extends Command {
  ...
  private async getConfigSourceFile(userConfigPath: string) {
    ...
    return {
      ...
      sourceFile,
      importModules: ["@karnak19/openapi-codegen-cli"],
    };
  }
    ...
    if (this.dryRun) {
      ...
      try {
        ...
        if (!hasCli && !hasTsPlugin) {
          nextSteps.push(
            "npm install -D @karnak19/openapi-codegen-{cli,typescript}"
          );
        } else if (!hasCli) {
          nextSteps.push("npm install -D @karnak19/openapi-codegen-cli");
        } else if (!hasTsPlugin) {
          nextSteps.push("npm install -D @karnak19/openapi-codegen-typescript");
        }
      } catch {
        nextSteps.push(
          "npm install -D @karnak19/openapi-codegen-{cli,typescript}"
